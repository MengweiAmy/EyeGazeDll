									...
									/*--------------------------------------------------------------------------*/
{
	...
	{
		char ch; // used only to eliminate code analysis warning for ignored return value
		//wprintf(TEXT("iMinimumFixSamples %i >= RING_SIZE %i\n"),
		//	iMinimumFixSamples, RING_SIZE);
		//wprintf(TEXT("Press any key to terminate..."));
		ch = _getch();
		exit(99);
	}
